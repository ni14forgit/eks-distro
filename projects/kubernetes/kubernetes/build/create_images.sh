...
if [ "$PUSH" != "true" ]; then
    ...
    build::images::pause_tar $PAUSE_BUILDER_IMAGE $PAUSE_BASE_IMAGE $IMAGE_TAG $PAUSE_IMAGE ${OUTPUT_DIR}/${RELEASE_BRANCH}/pause $BIN_DIR $SKIP_ARM
else
    build::images::push $RELEASE_BRANCH $GO_RUNNER_IMAGE $KUBE_PROXY_BASE_IMAGE $REPOSITORY_BASE $REPO_PREFIX $IMAGE_TAG $BIN_DIR
    build::images::pause_push $PAUSE_BUILDER_IMAGE $PAUSE_BASE_IMAGE $IMAGE_TAG $PAUSE_IMAGE ${OUTPUT_DIR}/${RELEASE_BRANCH}/pause 
fi
